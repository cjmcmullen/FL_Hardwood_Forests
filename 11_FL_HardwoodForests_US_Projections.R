...
input_base_dir <- "02_rasters/Future_USLayers"  # Base directory for rasters
model_base_dir <- "04_ENMs"  # Base directory for models
output_base_dir <- "08_FutureUSProjections"  # Base directory for output projections
...
for (model_dir in model_dirs) {
  ...
  Soil <- list.files("02_rasters/Current_USLayers", pattern = "*v2.asc", full.names = TRUE)
  Elev <- list.files("02_rasters/Current_USLayers", pattern = "*elev.asc", full.names = TRUE)
  ...
  projection_filename <- file.path(output_base_dir, paste0(species, "/", species, "_", model, "_", time_period, "_", ssp, "_ENM_Projection.asc"))
